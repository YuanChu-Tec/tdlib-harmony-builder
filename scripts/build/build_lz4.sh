...
# 查找源码目录
SOURCE_DIR=$(find_source_dir "lz4" "$LZ4_VERSION")
if [[ -z "$SOURCE_DIR" ]] || [[ ! -d "$SOURCE_DIR" ]]; then
    log_error "未找到 LZ4 源码，请下载 LZ4 源码压缩包并放到 src/downloads/ 目录"
    exit 1
fi
...
# 检查是否已编译
if check_already_built "lz4" "$ARCH"; then
    log_info "LZ4 已经编译安装"
    exit 0
fi

# 创建构建目录
BUILD_DIR=$(create_build_dir "lz4" "$ARCH")
cd "$BUILD_DIR" || exit 1
...
# 配置 CMake
run_command \
    "\"$CMAKE_CMD\" \"$SOURCE_DIR/build/cmake\" \
    ...
    "${LOGS_DIR}/build/lz4_${ARCH}_configure.log" \
    "配置 LZ4"
...
run_command \
    ...
    "${LOGS_DIR}/build/lz4_${ARCH}_build.log" \
    "编译 LZ4"
...
run_command \
    ...
    "${LOGS_DIR}/build/lz4_${ARCH}_install.log" \
    "安装 LZ4"
...
# 编译后验证
if ! verify_build_result "lz4" "$ARCH" "liblz4.a" "lz4.h"; then
    log_error "LZ4 编译验证失败"
    exit 1
fi

log_success "LZ4 编译安装完成: $ARCH"
